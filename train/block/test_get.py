        ...
        if args.OD_plot[0]:
            ...
            for i in range(min(len(test_all),args.OD_plot[1])):
                ...
                if len(choose)==0:
                    print('!没有任何达标的预测框!')
                    break
                choose = choose[0:min(len(choose),100)] #合理更改保留的框数可避免错检
                ...
                img = test_all[i].cpu().numpy()
                for j in range(len(all_choose)):
                    cv2.rectangle(img, (all_choose[j][0],all_choose[j][1]), (all_choose[j][2],all_choose[j][3]),
                                  color=(0,255,0), thickness=2)
                cv2.imshow(name_all[i]+': 99 frame', img)
                cv2.waitKey(0)
                ...
                choose=choose[mask_choose].type(torch.int32).cpu().numpy()
                img = test_all[i].cpu().numpy()
                for j in range(len(choose)):
                    cv2.rectangle(img, (choose[j][0],choose[j][1]), (choose[j][2],choose[j][3]),color=(0,255,0), thickness=2)
